...
- 정식 배포 기준은 `main`에만 둔다.
- Claude·Co-work 수정본은 작업 브랜치에 커밋하고 `v0.63.70-claude-debug.N` 형식으로 별도 기록한다.
- 각 디버그 저장 시 커밋 해시, 변경 요약, 검사 결과, 미리보기 주소를 PR에 남긴다.
- 디버그 미리보기 확인 전에는 `main`에 병합하지 않는다.
- 현재 자산 캐시 기준은 `0.63.70-debug`이며, 일정 프레임 URL에도 이 번호가 붙는다. 같은 파일을 교체할 때는 이 번호도 올려 브라우저의 구형 이미지 캐시를 차단한다.

## 활동 동작 v2 확정

- 글공부: 낮은 책상 위 종이에 붓으로 쓰기
- 셈하기: 낮은 책상 위 주판알 튕기기
- 마당쓸기: 양손으로 빗자루를 잡고 좌·중앙·우로 쓸기
- 장터 심부름: 채소가 든 장바구니를 들고 좌측에서 우측으로 걷기
- 약초 뽑기: 뿌리 가까이를 잡아 뽑은 뒤 바구니에 담기
- 집 청소: 손걸레를 손바닥으로 눌러 앞뒤로 닦기
- 농가 일손돕기: 구멍 파기 → 모종 세우기 → 흙 다독이기

생성 원본은 `assets/characters/seonhwa/schedule-actions/source-pixel-v2/`에 보존한다. 게임용 프레임은 `tools/build_action_sheets_v2.mjs`로 320×320 투명 PNG로 빌드한다. 잠겨 있는 고급 교육·직업도 `canonicalActivityAliases`에 미리 연결되어, 해금 직후 동일한 9세 픽셀 베이스가 사용된다.
