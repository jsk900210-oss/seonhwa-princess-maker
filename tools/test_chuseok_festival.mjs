...
for(const asset of ['app.js','style.css','schedule.css'])assert.ok(html.includes(`${asset}?v=0.64.167-debug`),`${asset} 캐시 키가 표시 버전과 같아야 합니다.`);
...
assert.match(app,/moonlightPixelMotionMap=\{[\s\S]*bow:\['manners-pixel-1\.png'[\s\S]*walk:\['\.\.\/schedule-base\/walk-1-v2\.png','\.\.\/schedule-base\/walk-2-v2\.png'[\s\S]*finish:\['manners-pixel-3\.png'/,'절·걷기·마지막 인사는 정돈된 쯔꾸르 전용 프레임이어야 합니다.');
