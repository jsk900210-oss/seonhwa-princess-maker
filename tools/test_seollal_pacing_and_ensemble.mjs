...
assert.match(app,/minimumStay=beat===4\?2500:beat<=1\?1600:700/,'출품작 확인은 2.5초, 시작 대화는 1.6초 이상 보여야 합니다.');
...
assert.match(app,/preparing=beat===4,vote=beat===5,result=beat===6,guardianResult=beat===7/,'8인 그림 장면 다음에는 심사·결과·축하로 바로 이어져야 합니다.');
assert.match(app,/function sehwaPreparationEnsemble\(session\)/,'빈 준비 장면 대신 참가자 8명의 화구 준비 장면이 필요합니다.');
assert.match(app,/\$\{preparing\?sehwaSubmissionCard\(session\):''\}/,'출품작 카드는 제출 장면에만 보여야 합니다.');
...
assert.doesNotMatch(app,/const hero=drawing/,'선화가 혼자 그림을 그리는 장면은 실행 경로에 남아 있으면 안 됩니다.');
assert.doesNotMatch(app,/is-tsukuru-painting/,'혼자 그림을 그리는 쯔꾸르 장면은 실행 경로에 남아 있으면 안 됩니다.');
...
assert.doesNotMatch(app,/선화가 자리에 앉아 세화를 그리기 시작했어요/,'삭제한 혼자 그림 그리기 장면의 서술이 남아 있으면 안 됩니다.');
...
console.log('PASS: 설날 수동 진행·출품작 확인·스테이터스 응답·대화 배치');
