...
for(const activityId of ['kitchenhelp','childcare','painting','music','dance','sewing','copying','woodwork','loomwork','farmwork','swordsmanship','spellcraft','classics','masonry','clinichelp','innhelp','ferryhelp','merchanthelp','accounting','tutoring']){
  ...
  assert.ok(manifest.existingHeroFrames.every(path=>path.includes(`characters/seonhwa/schedule-actions/v2/${heroFrameId}-pixel-v2-`)),'일정별 승인된 선화 3프레임을 참조해야 합니다.');
...
assert.ok(app.includes('const heroStart=travelsRight?-25:125')&&app.includes('*travelProgress*150'),'농가 닭 추격은 한쪽 화면 밖에서 반대편 화면 밖까지 완주해야 합니다.');
assert.ok(app.includes('const chaseGap=38'),'커진 닭과 선화의 불투명 영역이 겹치지 않는 안전 간격을 유지해야 합니다.');
...
assert.match(css,/action-farmwork\.schedule-layered \.schedule-scene-layer\.layer-pattern\.fail-b\{[\s\S]*?width:84px!important;/,'닭은 추격 장면에서 알아보기 쉬운 크기여야 합니다.');
