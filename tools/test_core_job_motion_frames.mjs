...
for(const job of jobs){
  ...
  const hashes=[1,2,3].map(frame=>crypto.createHash('sha256').update(fs.readFileSync(path.join(root,'assets/characters/seonhwa/job-actions',`${job}-${frame}.png`))).digest('hex'));
