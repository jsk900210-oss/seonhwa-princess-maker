...
for (const activity of modular) {
  for (let frame = 1; frame <= 3; frame += 1) {
    assert.ok(existsSync(join(root, 'assets', 'characters', 'seonhwa', 'schedule-actions', 'v2', `${activity}-pixel-v2-${frame}.png`)), `${activity} 공통 3프레임이 필요합니다.`);
  }
}

assert.ok(app.includes('schedule-actions/v2/${file}'), '일정은 승인된 공통 선화 프레임을 사용해야 합니다.');
...
assert.ok(app.includes("rest:frameTriplet('tea-pixel-v2')"), '차 마시기는 승인된 휴식 3프레임을 사용해야 합니다.');
