...
const outfits = [
  ...
  {id:'age18-ethnic-stage-v3',age:9,ageEnd:18,assetAge:18,name:'월백 무대 의상',price:0,tone:'캐시',category:'cash',forSale:false,seasons:['봄','여름','가을','겨울'],situations:['shopping','vacation'],change:{charm:20,reputation:14,arts:9,virtue:-2,stress:3}},
  {id:'age18-cash-ember-ethnic',age:9,ageEnd:18,assetAge:18,name:'홍염 자수 무대복',price:0,tone:'캐시',category:'cash',forSale:false,seasons:['봄','여름','가을','겨울'],situations:['shopping','vacation'],change:{charm:21,reputation:13,arts:10,virtue:-2}},
  {id:'age18-cash-solar-ceremony',age:9,ageEnd:18,assetAge:18,name:'태양 백금 예복',price:0,tone:'캐시',category:'cash',forSale:false,seasons:['봄','여름','가을','겨울'],situations:['shopping','vacation'],change:{charm:18,reputation:18,manners:9,arts:6}},
  {id:'age18-cash-ink-scholar',age:9,ageEnd:18,assetAge:18,name:'묵학 비상 예복',price:0,tone:'캐시',category:'cash',forSale:false,seasons:['봄','여름','가을','겨울'],situations:['reading','shopping','vacation'],change:{study:12,arts:12,reputation:12,charm:10}},
  {id:'age18-cash-starlight-pink',age:9,ageEnd:18,assetAge:18,name:'별빛 유리 무대복',price:0,tone:'캐시',category:'cash',forSale:false,seasons:['봄','여름','가을','겨울'],situations:['shopping','vacation'],change:{charm:24,reputation:16,arts:13,stress:-2}},
  {id:'age18-cash-rose-paisley',age:9,ageEnd:18,assetAge:18,name:'장미 페이즐리 무대복',price:0,tone:'캐시',category:'cash',forSale:false,seasons:['봄','여름','가을','겨울'],situations:['shopping','vacation'],change:{charm:23,reputation:15,arts:12,virtue:-3}},
...
function renderShopPanel(tab='food',marketMode=marketShoppingActive,outfitCategory='general'){
  ...
  const outfitCards=visibleOutfits.map(outfit=>{const premium=isPremiumOutfit(outfit),cash=isCashOutfit(outfit),locked=cash||!outfitAvailable(outfit)||owned.has(outfit.id)||game.money<outfit.price;return `<button class="shop-card outfit-card visual-card ${outfitAvailable(outfit)?'available':''} ${premium?'premium':''} ${cash?'cash':''} ${locked?'locked':''}" data-outfit-preview="${outfit.id}" aria-label="${outfit.name} 미리보기"><img src="../assets/characters/seonhwa/wardrobe/age-${String(outfitAssetAge(outfit)).padStart(2,'0')}/${outfit.id}.png" alt="${outfit.name}"><b>${outfit.name}</b><span>${cash?'출시 예정':`${outfit.price}냥`}</span><small>${cash?'캐시 의상 · ':premium?'고급 의상 · ':''}${outfitAgeLabel(outfit)} · ${outfit.seasons.join('·')}<br>${formatChanges(outfit.change)}${owned.has(outfit.id)?'<br>보유 중':''}</small></button>`;}).join('');
  ...
  panelBody.innerHTML=`<div class="shop-greeting"><img src="${keeper.image}" alt="${keeper.name}"><div><b>${keeper.name}</b><p>${keeper.greeting}</p></div></div><div class="shop-money">보유 은전 <b>${game.money.toLocaleString()}냥</b></div>${marketMode?'':`<div class="shop-tabs"><button data-shop-tab="food" class="${tab==='food'?'on':''}">주막</button><button data-shop-tab="outfit" class="${tab==='outfit'?'on':''}">한복점</button></div>`}${outfitCategoryTabs}<h3 class="shop-list-title">${tab==='food'?`음식 메뉴 · ${foods.length}종`:`${categoryName} 의상 · ${visibleOutfits.length}벌`}</h3><div class="shop-grid">${tab==='food'?foodCards:outfitCards}</div><button id="shopBack">${marketMode?'저잣거리로 나가기':'일정으로 돌아가기'}</button>`;
...
function showOutfitPreview(id){
  ...
  normalizeInventory();
  const owned=game.items.some(item=>item.type==='outfit'&&item.id===id),cash=isCashOutfit(outfit),ageLocked=!outfitAvailable(outfit),insufficient=game.money<outfit.price;
  const reason=cash?'추후 캐시 아이템으로 출시될 예정입니다.':owned?'이미 구매한 의상입니다.':ageLocked?`${outfitAgeLabel(outfit)}에 구매할 수 있습니다.`:insufficient?'보유 은전이 부족합니다.':'미리 입어본 뒤 구매할 수 있습니다.';
  const grade=cash?'캐시 의상':isPremiumOutfit(outfit)?'고급 의상':'일반 의상';
  panelBody.insertAdjacentHTML('beforeend',`<div class="outfit-preview-backdrop" id="outfitPreview"><section class="outfit-preview-card ${cash?'cash-preview':''}" role="dialog" aria-modal="true" aria-label="${outfit.name} 미리보기"><button class="outfit-preview-close" id="outfitPreviewClose" aria-label="미리보기 닫기">×</button><div class="outfit-preview-image"><img src="../assets/characters/seonhwa/wardrobe/age-${String(outfitAssetAge(outfit)).padStart(2,'0')}/${outfit.id}.png" alt="${outfit.name} 전신 미리보기"></div><div class="outfit-preview-info"><small>${grade}</small><h3>${outfit.name}</h3><p>${outfitAgeLabel(outfit)} · ${outfit.seasons.join('·')}<br>${formatChanges(outfit.change)}</p><b>${cash?'출시 예정':`${outfit.price.toLocaleString()}냥`}</b><em>${reason}</em><button id="outfitPreviewBuy" ${cash||owned||ageLocked||insufficient?'disabled':''}>${cash?'캐시 아이템 준비 중':owned?'구매 완료':'이 의상 구매하기'}</button></div></section></div>`);
