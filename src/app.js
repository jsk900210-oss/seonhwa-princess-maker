...
function showGuardianStory(){
  ...
  const image=document.querySelector('#guardianStoryImage'),focus=document.querySelector('#guardianStoryFocus');clearTimeout(guardianSceneTimer);image.classList.add('scene-fading');focus.classList.remove('focus-playing');copy.classList.add('is-changing');
  guardianSceneTimer=setTimeout(()=>{image.src=scene.image;image.alt=scene.alt;focus.src=scene.image;image.classList.remove('scene-fading','scene-changing');void image.offsetWidth;image.classList.add('scene-changing');void focus.offsetWidth;focus.classList.add('focus-playing');document.querySelector('#guardianStoryChapter').textContent=scene.chapter;document.querySelector('#guardianStoryText').textContent=scene.text;document.querySelector('#guardianStoryNext').textContent=guardianStoryIndex===guardianStoryScenes.length-1?'신수 선택하기':'다음';copy.classList.remove('is-changing');},240);
}
