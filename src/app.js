...
const isPremiumOutfit=outfit=>outfit.id.startsWith('premium-')||outfit.tone==='고급';
function renderShopPanel(tab='food',marketMode=marketShoppingActive,outfitCategory='general'){
  ...
  const visibleOutfits=outfits.filter(outfit=>isPremiumOutfit(outfit)===(outfitCategory==='premium'));
  const outfitCards=visibleOutfits.map(outfit=>{const premium=isPremiumOutfit(outfit),locked=!outfitAvailable(outfit)||owned.has(outfit.id)||game.money<outfit.price;return `<button class="shop-card outfit-card visual-card ${outfitAvailable(outfit)?'available':''} ${premium?'premium':''} ${locked?'locked':''}" data-outfit-preview="${outfit.id}" aria-label="${outfit.name} 미리보기"><img src="../assets/characters/seonhwa/wardrobe/age-${String(outfit.age).padStart(2,'0')}/${outfit.id}.png" alt="${outfit.name}"><b>${outfit.name}</b><span>${outfit.price}냥</span><small>${premium?'고급 의상 · ':''}${outfitAgeLabel(outfit)} · ${outfit.tone} · ${outfit.seasons.join('·')}<br>${formatChanges(outfit.change)}${owned.has(outfit.id)?'<br>구매 완료':''}</small></button>`;}).join('');
  const outfitCategoryTabs=tab==='outfit'?`<div class="outfit-shop-tabs" role="tablist" aria-label="의상 등급"><button data-outfit-category="general" class="${outfitCategory==='general'?'on':''}">일반 의상</button><button data-outfit-category="premium" class="${outfitCategory==='premium'?'on':''}">고급 의상</button></div>`:'';
  panelBody.innerHTML=`<div class="shop-greeting"><img src="${keeper.image}" alt="${keeper.name}"><div><b>${keeper.name}</b><p>${keeper.greeting}</p></div></div><div class="shop-money">보유 은전 <b>${game.money.toLocaleString()}냥</b></div>${marketMode?'':`<div class="shop-tabs"><button data-shop-tab="food" class="${tab==='food'?'on':''}">주막</button><button data-shop-tab="outfit" class="${tab==='outfit'?'on':''}">한복점</button></div>`}${outfitCategoryTabs}<h3 class="shop-list-title">${tab==='food'?`음식 메뉴 · ${foods.length}종`:`${outfitCategory==='premium'?'고급':'일반'} 의상 · ${visibleOutfits.length}벌`}</h3><div class="shop-grid">${tab==='food'?foodCards:outfitCards}</div><button id="shopBack">${marketMode?'저잣거리로 나가기':'일정으로 돌아가기'}</button>`;
...
function showOutfitPreview(id){
  ...
  normalizeInventory();
  const owned=game.items.some(item=>item.type==='outfit'&&item.id===id),ageLocked=!outfitAvailable(outfit),insufficient=game.money<outfit.price;
  const reason=owned?'이미 구매한 의상입니다.':ageLocked?`${outfitAgeLabel(outfit)}에 구매할 수 있습니다.`:insufficient?'보유 은전이 부족합니다.':'미리 입어본 뒤 구매할 수 있습니다.';
  panelBody.insertAdjacentHTML('beforeend',`<div class="outfit-preview-backdrop" id="outfitPreview"><section class="outfit-preview-card" role="dialog" aria-modal="true" aria-label="${outfit.name} 미리보기"><button class="outfit-preview-close" id="outfitPreviewClose" aria-label="미리보기 닫기">×</button><div class="outfit-preview-image"><img src="../assets/characters/seonhwa/wardrobe/age-${String(outfit.age).padStart(2,'0')}/${outfit.id}.png" alt="${outfit.name} 전신 미리보기"></div><div class="outfit-preview-info"><small>${isPremiumOutfit(outfit)?'고급 의상':'일반 의상'}</small><h3>${outfit.name}</h3><p>${outfitAgeLabel(outfit)} · ${outfit.seasons.join('·')}<br>${formatChanges(outfit.change)}</p><b>${outfit.price.toLocaleString()}냥</b><em>${reason}</em><button id="outfitPreviewBuy" ${owned||ageLocked||insufficient?'disabled':''}>${owned?'구매 완료':'이 의상 구매하기'}</button></div></section></div>`);
