...
const scheduleAssetRevision='0.64.73-debug';
...
function sehwaPreparationEnsemble(session){
  if(sehwaAssetAge()==='13')return `<span class="sehwa-preparation-sequence" role="img" aria-label="여덟 참가자가 화지를 놓고 먹을 간 뒤 붓끝을 정돈하는 모습">${[1,2,3].map((frame,index)=>`<img src="../assets/events/holidays/sehwa-contest/preparation-scene/age-13/preparation-${frame}.png?v=${scheduleAssetRevision}" alt="" style="--preparation-frame:${index}">`).join('')}</span>`;
