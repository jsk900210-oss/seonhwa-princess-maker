...
const outfits = [
  ...
  {id:'age18-ethnic-stage-v3',age:18,ageEnd:18,name:'월백 무대 의상',price:0,tone:'캐시',category:'cash',forSale:false,seasons:['봄','여름','가을','겨울'],situations:['shopping','vacation'],change:{charm:20,reputation:14,arts:9,virtue:-2,stress:3}},
  {id:'age18-cash-ember-ethnic',age:18,ageEnd:18,name:'홍염 자수 무대복',price:0,tone:'캐시',category:'cash',forSale:false,seasons:['봄','여름','가을','겨울'],situations:['shopping','vacation'],change:{charm:21,reputation:13,arts:10,virtue:-2}},
...
function showOutfitPreview(id){
  ...
  panelBody.insertAdjacentHTML('beforeend',`<div class="outfit-preview-backdrop" id="outfitPreview"><section class="outfit-preview-card ${cash?'cash-preview':''}" role="dialog" aria-modal="true" aria-label="${outfit.name} 미리보기"><button class="outfit-preview-close" id="outfitPreviewClose" aria-label="미리보기 닫기">×</button><div class="outfit-preview-image"><img src="../assets/characters/seonhwa/wardrobe/age-${String(outfit.age).padStart(2,'0')}/${outfit.id}.png" alt="${outfit.name} 전신 미리보기"></div><div class="outfit-preview-info"><small>${grade}</small><h3>${outfit.name}</h3><p>${outfitAgeLabel(outfit)} · ${outfit.seasons.join('·')}<br>${formatChanges(outfit.change)}</p><b>${cash?'출시 예정':`${outfit.price.toLocaleString()}냥`}</b><em>${reason}</em><button id="outfitPreviewBuy" ${cash||owned||ageLocked||insufficient?'disabled':''}>${cash?'캐시 아이템 준비 중':owned?'구매 완료':'이 의상 구매하기'}</button></div></section></div>`);
