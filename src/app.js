...
function renderShopPanel(tab='food',marketMode=marketShoppingActive){
  ...
  const outfitCards=outfits.map(outfit=>{const premium=outfit.id.startsWith('premium-')||outfit.tone==='고급';return `<button class="shop-card outfit-card visual-card ${outfitAvailable(outfit)?'available':''} ${premium?'premium':''}" data-outfit="${outfit.id}" ${!outfitAvailable(outfit)||owned.has(outfit.id)||game.money<outfit.price?'disabled':''}><img src="../assets/characters/seonhwa/wardrobe/age-${String(outfit.age).padStart(2,'0')}/${outfit.id}.png" alt="${outfit.name}"><b>${outfit.name}</b><span>${outfit.price}냥</span><small>${premium?'고급 의상 · ':''}${outfitAgeLabel(outfit)} · ${outfit.tone} · ${outfit.seasons.join('·')}<br>${formatChanges(outfit.change)}${owned.has(outfit.id)?'<br>구매 완료':''}</small></button>`;}).join('');
  panelBody.innerHTML=`<div class="shop-greeting"><img src="${keeper.image}" alt="${keeper.name}"><div><b>${keeper.name}</b><p>${keeper.greeting}</p></div></div><div class="shop-money">보유 은전 <b>${game.money.toLocaleString()}냥</b></div>${marketMode?'':`<div class="shop-tabs"><button data-shop-tab="food" class="${tab==='food'?'on':''}">주막</button><button data-shop-tab="outfit" class="${tab==='outfit'?'on':''}">한복점</button></div>`}<h3 class="shop-list-title">${tab==='food'?`음식 메뉴 · ${foods.length}종`:`한복 상품 · ${outfits.length}벌`}</h3><div class="shop-grid">${tab==='food'?foodCards:outfitCards}</div><button id="shopBack">${marketMode?'저잣거리로 나가기':'일정으로 돌아가기'}</button>`;
