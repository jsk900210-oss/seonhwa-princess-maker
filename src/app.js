...
let guardianStoryIndex=0,guardianSceneTimer=null,guardianAbilityTimer=null,selectedGuardianType=null,introDialogueQueue=[],introDialogueIndex=0;
const statGroups = [
...
function showGuardianStory(){
  ...
  const image=document.querySelector('#guardianStoryImage'),focus=document.querySelector('#guardianStoryFocus'),frames=scene.frames||[scene.image];clearTimeout(guardianSceneTimer);clearTimeout(guardianAbilityTimer);image.classList.add('scene-fading');focus.classList.remove('focus-playing','ability-frame');copy.classList.add('is-changing');
  guardianSceneTimer=setTimeout(()=>{image.src=frames[0];image.alt=scene.alt;focus.src=frames[1]||frames[0];image.classList.remove('scene-fading','scene-changing');void image.offsetWidth;image.classList.add('scene-changing');void focus.offsetWidth;focus.classList.add('focus-playing');if(frames[2])guardianAbilityTimer=setTimeout(()=>{focus.classList.add('ability-frame');focus.src=frames[2];},4050);document.querySelector('#guardianStoryChapter').textContent=scene.chapter;document.querySelector('#guardianStoryText').textContent=scene.text;document.querySelector('#guardianStoryNext').textContent=guardianStoryIndex===guardianStoryScenes.length-1?'신수 선택하기':'다음';copy.classList.remove('is-changing');},240);
}
